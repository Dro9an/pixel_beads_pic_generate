      ...
      <section class="control-section">
        <div class="controls">
          ...
          <div style="width: 100%;display: flex;flex-direction: column;align-items: center;gap: 10px">
            <h3>豆子是MARD多少色呢？👇</h3>
            <el-radio-group v-model="colorConfig.count" style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 5px;">
...
const generateAndDownloadPixelArt = async () => {
  ...
  try {
    // 像素化处理图像
    const result = await pixelateImage(imageUrl.value, currentSize.value.width, currentSize.value.height)
    
    // 将像素颜色匹配到拼豆颜色
    const matchedPixels = result.pixels.map(row => {
    ...
    for (let y = 0; y < matchedPixels.length; y++) {
      for (let x = 0; x < matchedPixels[y].length; x++) {
        ...
        ctx.fillStyle = pixel.rgb.reduce((acc, val) => acc + val, 0) > 300 ? '#000000' : '#ffffff'
    ...
    ElMessage.error('坏了出问题了，你把这个截图给我看看我马上就去修！\n'+error)
  } finally {
